...
def hapblock_hapblock_error_rate(truth_file, truth_vcf_file, assembly_file, frag_file, vcf_file, contig_size_file):
    ...
    err = error_rate_calc(t_blocklist, a_blocklist, vcf_file, frag_file, contig_size_file, chrom='')
    return err
...
def hapblock_vcf_error_rate(assembly_file, frag_file, vcf_file, phased_vcf_file, contig_size_file, largest_blk_only=False):
    ...
    err = error_rate_calc(t_blocklist, a_blocklist, vcf_file, frag_file, contig_size_file, chrom=CHROM)
    return err

# num_covered should be the number of SNPs with coverage in the fragment matrix file
def error_rate_calc(t_blocklist, a_blocklist, vcf_file, frag_file, contig_size_file, chrom, phase_set=None, ):
    ...
    try:
        lst = [switch_loc,mismatch_loc,missing_loc]
        names = ['switch_loc', 'mismatch_loc', 'missing_loc']
        for i in range(len(lst)):
            outfile=f'Make_Vcf/step3_hapcut/step4_compare_with_refphasing/{names[i]}.bed'
            with open(outfile, 'a') as f:
                items = lst[i]
                for item in items:
                    f.write(f"{chrom}\t{item}\t{int(item)+1}\n")
